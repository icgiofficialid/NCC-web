...
import { NCC_ACCENT } from "@/components/ncc/NccData";
...
const NccContact = () => {
  ...
  const handleSend = () => {
    ...
    const subject = encodeURIComponent(`Contact from ${form.name} - NCC`);
        ...
        <SectionReveal>
          <div
            ...
            style={{
              ...
              background: "linear-gradient(135deg, hsl(32 100% 55% / 0.04) 0%, hsl(350 90% 55% / 0.04) 100%)",
            }}
          >
            <div className="relative max-w-3xl space-y-5">
              <div
                className="inline-flex items-center gap-3 rounded-full border px-4 py-2 text-sm uppercase tracking-[0.24em]"
                style={{ borderColor: "hsl(32 100% 55% / 0.35)", background: "hsl(32 100% 55% / 0.08)", color: AMBER }}
              >
                <Mail className="h-4 w-4" />
                {lang === "en" ? "Contact" : "Kontak"}
              </div>
              <h1 className="font-display text-4xl md:text-5xl">
                {lang === "en" ? "Connect with the NCC Team" : "Terhubung dengan Tim NCC"}
              </h1>
              <p className="max-w-xl leading-8 text-muted-foreground">
                {lang === "en"
                  ? "Reach out for registration support, collaboration, or any questions about NCC."
                  : "Hubungi kami untuk dukungan pendaftaran, kolaborasi, atau pertanyaan tentang NCC."}
              </p>
            </div>
          </div>
        </SectionReveal>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="rounded-2xl overflow-hidden border border-border/50 grid md:grid-cols-[1fr_1.2fr]">
            ...
            <div className="bg-background p-8 md:p-10 flex flex-col gap-6">
              <div>
                <h3 className="text-xl font-bold font-display mb-2" style={{ color: AMBER }}>
              ...
              <div className="space-y-4 text-sm text-muted-foreground">
                ...
                <div className="flex items-start gap-3">
                  <PhoneCall className="h-4 w-4 shrink-0 mt-1" style={{ color: AMBER }} />
                ...
                <div className="flex items-start gap-3">
                  <Globe className="h-4 w-4 shrink-0 mt-1" style={{ color: AMBER }} />
                ...
                <div className="flex gap-3">
                  {[
                    ...
                    <a key={i} href={href} target="_blank" rel="noopener noreferrer"
                      ...
                      style={{ borderColor: "hsl(32 100% 55% / 0.3)", background: "hsl(32 100% 55% / 0.08)", color: AMBER }}>
            ...
            <div
              ...
              style={{ background: "linear-gradient(135deg, hsl(32 40% 10%), hsl(350 40% 12%))" }}
            >
              <h3 className="text-xl font-bold text-white font-display">Contact us</h3>
              <input name="name" value={form.name} onChange={handleChange}
                placeholder={lang === "en" ? "Name" : "Nama"}
                className="w-full rounded-xl border border-white/15 bg-white/5 text-white placeholder:text-white/30 px-5 py-3 text-sm focus:outline-none focus:border-amber-400/40 transition" />
              <input name="email" value={form.email} onChange={handleChange}
                placeholder="Email" type="email"
                className="w-full rounded-xl border border-white/15 bg-white/5 text-white placeholder:text-white/30 px-5 py-3 text-sm focus:outline-none focus:border-amber-400/40 transition" />
              <textarea name="message" value={form.message} onChange={handleChange}
                placeholder={lang === "en" ? "Message" : "Pesan"} rows={5}
                className="w-full rounded-xl border border-white/15 bg-white/5 text-white placeholder:text-white/30 px-5 py-3 text-sm focus:outline-none focus:border-amber-400/40 transition resize-none" />
